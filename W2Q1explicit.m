function [matrixu,matrixofcorrectsolutions] = W2Q1explicit(xbeginning,xend,tbeginning,tend,numberofpointsinx,numberofpointsint,eta,gx1,gxend,f,actualsol)


% for solving equations of the form u_t - u_xx = f(x,t)


%inputs

%xbeginning =0 ;
%xend = 2*pi;
%tbeginning =0 ;
%tend = 1;
%numberofpointsinx = 20;
%numberofpointsint= 10;
%eta = @(x) sin(x) ; %.... ;
%alpha1 =1;
%beta1 =0;
%alpha2 =1;
%beta2 =0;
%zeta1 = @(x,t) exp(t);
%zeta2 = @(x,t) exp(t);
%f = @(x,t) 0;
%actualsol = @(x,t) exp(t)*sin(x);

matrixu = zeros(numberofpointsint+1, numberofpointsinx+1); % X's across t down (columns)

dx = (xend-xbeginning)/numberofpointsinx;
dt = (tend-tbeginning)/numberofpointsint;

i=1;
j=1;

% Mesh points (uniform)

pointx = [];
pointt = [];
pointx(1) = xbeginning;
pointt(1) = tbeginning;

        for i=1:(numberofpointsinx)
        pointx(i+1) = xbeginning + (i)*dx;
        end
        
        for j=1:(numberofpointsint)
        pointt(j+1) = tbeginning + (j)*dt;
        end
        
        % - Initial conditions for eta (t = 0)
        
        for i = 2:numberofpointsinx
        matrixu(1,i) = eta(pointx(i));
        end

        % Boundary conditions 

        matrixu(1,1) = gx1;
        matrixu(1,numberofpointsinx+1) = gxend;
        
        %Build the Euler explicit algorithm.
        
        for j = 1:numberofpointsint
                for i = 2:numberofpointsinx
                    matrixu(j+1,i) = matrixu(j,i) + (dt)*f(pointx(i),pointt(j)) + (dt/(dx^2))*(matrixu(j,i-1) - 2*matrixu(j,i) + matrixu(j,i+1));
                    matrixu(j+1,1) = gx1;
                    matrixu(j+1,numberofpointsinx+1) = gxend;
                end
        end
        
        %Call the results
       
           %for j = 1:numberofpointsint+1
            %   for i = 1:numberofpointsinx+1
             %    disp("At time step = " + (j-1) + "i,e at t = " + pointt(j) + "at space step =' + (i-1) + 'i,e x = " + pointx(i) + "the approximation of U(x,t) =" + disp(matrixu(j,i)) )
              % end
           %end
        
        %first row = timestep 0
        
        %second row = timestep 1 => time tbeginning + dt
        
        matrixofcorrectsolutions = zeros(numberofpointsint+1, numberofpointsinx+1);
        for j = 1:numberofpointsint +1
           for i = 1:numberofpointsinx +1
            matrixofcorrectsolutions(j,i) = actualsol(pointx(i),pointt(j));   
           end
        end
        
        %Calculate the error norm
        
        errormatrix = abs(matrixofcorrectsolutions-matrixu);
        errormatrixtable1 = dx*(errormatrix).^2;
        for j = 1:numberofpointsint+1
            rowjsum(j) = (sum(errormatrixtable1(j,1:numberofpointsinx+1)))^0.5;
        end
        maxerror = max(rowjsum);
        
        
        

 
end









